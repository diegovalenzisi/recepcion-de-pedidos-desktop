...
const { existsSync, readFileSync, writeFileSync, appendFileSync, createWriteStream, unlink, mkdirSync, copyFileSync, cpSync, rmSync } = require('fs');
...
function spawnFacturacionProc(key, accountDir) {
  ...
  try {
    ...
    if (!nodeBin) {
      entry.status = 'error';
      entry.logs.push(`[${new Date().toLocaleTimeString('es-AR')}] [ERR] Node 16 AFIP no encontrado en resources/node-afip/node.exe`);
  ...
  ipcMain.handle('facturacion:install-deps', () => {
    ...
    return new Promise((resolve, reject) => {
      if (existsSync(srcModules)) {
        // ── Producción: copiar desde resources con robocopy ──────────────────
        ...
        // ── Producción sin deps bundleados → deps pack manual ────────────────
        const userData = app.getPath('userData');
        const depsPath = path.join(userData, 'facturacion', 'node_modules');
        reject(new Error(
          `Módulos AFIP no encontrados.\n` +
          `Copiá el deps pack a:\n${depsPath}\n\n` +
          `El deps pack debe contener: firebase-admin, soap, pdfkit, qrcode, dotenv, moment`
        ));
      }
    });
  });
