...
function HRPaymentsTab({ currentShift }) {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const enrichedPayments = paymentsData.map(p => ({
        ...p,
        employeeName: employeesMap[p.employeeId] || p.employeeName || 'Empleado no encontrado',
      }));
  ...
  const handleSave = async (paymentData, isEditing, allEmployees) => {
    try {
    ...
    >
      <Card className="shadow-xl rounded-xl flex-grow flex flex-col h-full bg-white">
        <CardHeader className="flex flex-row items-center justify-between">
          <CardTitle className="text-xl font-bold">Historial de Pagos a Empleados</CardTitle>
          <Button onClick={handleAdd}>
            <PlusCircle className="mr-2 h-4 w-4" /> Nuevo Pago a Empleado
          </Button>
        </CardHeader>
        <CardContent className="flex-grow overflow-auto p-4">
          {loading ? (
            ...
            <div className="text-center py-10 text-gray-500">
              ...
              <p className="text-sm mt-2">Haz clic en "Nuevo Pago a Empleado" para empezar.</p>
            </div>
          ) : (
            <Table>
              <TableHeader>
